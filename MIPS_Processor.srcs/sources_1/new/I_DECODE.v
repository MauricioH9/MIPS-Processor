...
module I_DECODE(
    ...
    input wire [31:0] if_id_instr,     // Instruction from IF/ID pipeline register
    input wire [31:0] if_id_npc,       // Next Program Counter from IF/ID pipeline register
    ...
    output wire [31:0] ex_reg_rs,      // Register RS value
    output wire [31:0] ex_reg_rt,      // Register RT value
    ...
    wire [31:0] reg_rs;                // Register RS value
    wire [31:0] reg_rt;                // Register RT value
    ...
    registers registers_ID (
        .clk(clk),
        .RegWrite(mem_wb_regwrite),    // Write enable from MEM/WB stage
        .rs(rs),                // Read address for rs
        .rt(rt),                // Read address for rt
        .rd(mem_wb_writereg),   // Write register address from MEM/WB stage
        .write_data(mem_wb_writedata), // Write data from MEM/WB stage
        .read_data_rs(reg_rs),           // Output data for rs
        .read_data_rt(reg_rt)            // Output data for rt
    );
